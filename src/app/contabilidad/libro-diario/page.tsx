...
const STATUS_CONFIG: Record<string, { icon: any; color: string; label: string }> = {
    borrador: { icon: CircleDashed, color: "text-amber-600 bg-amber-100 dark:text-amber-400 dark:bg-amber-950/50", label: "Borrador" },
    contabilizado: { icon: CheckCircle2, color: "text-emerald-600 bg-emerald-100 dark:text-emerald-400 dark:bg-emerald-950/50", label: "Contabilizado" },
    anulado: { icon: XCircle, color: "text-red-600 bg-red-100 dark:text-red-400 dark:bg-red-950/50", label: "Anulado" },
};
                ...
                <div className="flex justify-between items-start max-w-7xl mx-auto w-full">
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                            ...
                            <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">Libro Diario</h1>
                        </div>
                        <p className="text-slate-500 dark:text-zinc-400 font-medium pl-14">Registro cronológico de asientos contables — Partida Doble.</p>
                    </div>
                </div>
            </div>
                ...
                <div className="max-w-7xl mx-auto p-8 flex flex-col h-full">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-sm font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-wider mb-1">Contabilizados</p>
                        ...
                        <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-sm font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-wider mb-1">En Borrador</p>
                                <h3 className="text-2xl font-black text-amber-700 dark:text-amber-500">{totalDraft}</h3>
                            </div>
                        </div>
                    ...
                    <div className="flex-1 flex gap-6 min-h-0">
                        ...
                        <div className="flex-1 bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm overflow-hidden flex flex-col">
                            <div className="p-4 border-b border-slate-100 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-800/50 flex items-center justify-between">
                                <h3 className="font-bold text-slate-800 dark:text-zinc-200">Asientos Registrados</h3>
                                    ...
                                    entries.map((entry: any) => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`flex items-center gap-4 px-5 py-4 border-b border-slate-50 dark:border-zinc-800/50 cursor-pointer transition-all ${selectedEntry === entry._id
                                            ...
                                            >
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <p className="text-sm text-slate-700 dark:text-zinc-300 truncate">{entry.description}</p>
                                                    <div className="flex items-center gap-3 mt-1">
                                                        <span className="text-[10px] text-slate-400 dark:text-zinc-500">{entry.date}</span>
                                                        <span className="text-[10px] text-slate-400 dark:text-zinc-500">por {entry.createdByName}</span>
                                                    </div>
                                                </div>
                                                <div className="text-right shrink-0">
                                                    <span className="text-sm font-black text-slate-900 dark:text-white">{formatCurrency(entry.totalDebit)}</span>
                                                </div>
                                            </div>
                                        );
                                    })
                        ...
                        {selectedEntry && entryDetail && (
                            <div className="w-[400px] bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm overflow-hidden flex flex-col shrink-0">
                                <div className="p-4 border-b border-slate-100 dark:border-zinc-800 bg-amber-50 dark:bg-amber-900/20">
                                    <h3 className="font-bold text-amber-800 dark:text-amber-400 text-sm">{entryDetail.entryNumber}</h3>
                                ...
                                <div className="p-4 space-y-3 flex-1 overflow-y-auto">
                                    ...
                                    <div className="border-t border-slate-100 dark:border-zinc-800 pt-3">
                                        <h4 className="text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase mb-2">Líneas del Asiento</h4>
                                        <div className="space-y-1">
                                            {entryDetail.lines?.map((line: any, idx: number) => (
                                                <div key={idx} className={`flex items-center gap-2 text-xs py-1.5 px-2 rounded ${line.debit > 0 ? 'bg-blue-50/50 dark:bg-blue-900/10' : 'bg-red-50/50 dark:bg-red-900/10 pl-6'}`}>
                                                    <span className="font-mono text-slate-500 dark:text-zinc-500 w-12">{line.accountCode}</span>
                                                    <span className="flex-1 text-slate-700 dark:text-zinc-300 truncate">{line.accountName}</span>
                                                    {line.debit > 0 && <span className="font-bold text-blue-700 dark:text-blue-400">{formatCurrency(line.debit)}</span>}
                                                    {line.credit > 0 && <span className="font-bold text-red-600 dark:text-red-400">{formatCurrency(line.credit)}</span>}
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    <div className="border-t border-slate-200 dark:border-zinc-800 pt-3">
                                        <div className="flex justify-between text-xs font-bold">
                                            <span className="text-blue-700 dark:text-blue-400">Total Debe: {formatCurrency(entryDetail.totalDebit)}</span>
                                            <span className="text-red-600 dark:text-red-400">Total Haber: {formatCurrency(entryDetail.totalCredit)}</span>
                                        </div>
                                        <div className="mt-1 text-center">
                                            {Math.abs(entryDetail.totalDebit - entryDetail.totalCredit) < 0.01 ? (
                                                <Badge className="bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-400 text-[10px]">✓ Cuadrado</Badge>
                                            ) : (
                                                <Badge className="bg-red-100 dark:bg-red-950 text-red-700 dark:text-red-400 text-[10px]">✗ Descuadrado</Badge>
                                            )}
                                        </div>
                                    </div>
                                </div>
