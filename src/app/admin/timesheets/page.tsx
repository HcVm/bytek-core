...
export default function TimesheetsPage() {
    ...
    const statusColors: Record<string, string> = {
        draft: "bg-slate-100 dark:bg-zinc-800 text-slate-700 dark:text-zinc-300",
        approved: "bg-emerald-100 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400",
        invoiced: "bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400",
    };
                ...
                <div className="flex justify-between items-start max-w-7xl mx-auto w-full">
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                            ...
                            <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">Registro de Horas</h1>
                        </div>
                ...
                <div className="max-w-7xl mx-auto p-8 space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        ...
                        <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm">
                            <p className="text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase mb-1">Horas Facturables</p>
                            <h3 className="text-2xl font-black text-teal-600">{billableHours.toFixed(1)}h</h3>
                        </div>
                        <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm">
                            <p className="text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase mb-1">Valor Facturable</p>
                            <h3 className="text-2xl font-black text-slate-900 dark:text-white">{formatCurrency(billableAmount)}</h3>
                        </div>
                        <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm">
                            <p className="text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase mb-1">Registros</p>
                            <h3 className="text-2xl font-black text-slate-900 dark:text-white">{entries.length}</h3>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-2">
                        {["", "draft", "approved", "invoiced"].map(s => (
                            <Button key={s} variant={statusFilter === s ? "default" : "outline"} size="sm"
                                className={`text-xs ${statusFilter === s ? 'bg-teal-600 text-white' : 'dark:border-zinc-800 dark:text-zinc-400 dark:hover:bg-zinc-800'}`}
                        ...
                        {entries.length === 0 ? (
                            ...
                            <table className="w-full text-sm">
                                <thead className="bg-slate-50 dark:bg-zinc-800/50 border-b border-slate-200 dark:border-zinc-800">
                                    <tr>
                                        <th className="text-left px-5 py-3 text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase">Colaborador</th>
                                        <th className="text-left px-3 py-3 text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase">Proyecto</th>
                                        <th className="text-left px-3 py-3 text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase">Fecha</th>
                                        <th className="text-right px-3 py-3 text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase">Horas</th>
                                        <th className="text-left px-3 py-3 text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase">Descripción</th>
                                        <th className="text-right px-5 py-3 text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase">Estado</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-50 dark:divide-zinc-800">
                                    {entries.map((e: any) => (
                                        <tr key={e._id} className="hover:bg-slate-50/80 dark:hover:bg-zinc-800/50">
                                            <td className="px-5 py-3 font-medium text-slate-900 dark:text-white">{e.userName}</td>
                                            <td className="px-3 py-3 text-slate-600 dark:text-zinc-400">{e.projectName}</td>
                                            <td className="px-3 py-3 text-slate-600 dark:text-zinc-400">{e.date}</td>
                                            <td className="text-right px-3 py-3 font-mono font-bold text-slate-900 dark:text-zinc-100">
                                                {e.hours.toFixed(1)}h
                                                {e.billable && <FileBarChart className="w-3 h-3 text-teal-500 dark:text-teal-400 inline ml-1" />}
                                            </td>
                                            <td className="px-3 py-3 text-slate-500 dark:text-zinc-500 text-xs truncate max-w-[200px]">{e.description}</td>
                    ...
                    {showForm && (
                        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                            <div className="bg-white dark:bg-zinc-900 border dark:border-zinc-800 rounded-2xl shadow-2xl w-full max-w-lg p-6 space-y-4">
                                <div className="flex items-center justify-between mb-2">
                                    <h3 className="text-lg font-bold text-slate-900 dark:text-white">Registrar Horas</h3>
                                    <Button variant="ghost" size="icon" className="dark:text-zinc-400" onClick={() => setShowForm(false)}><X className="w-4 h-4" /></Button>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <Label className="text-xs font-bold text-slate-600 dark:text-zinc-400">Colaborador</Label>
                                    ...
                                    <div>
                                        <Label className="text-xs font-bold text-slate-600 dark:text-zinc-400">Proyecto</Label>
                                    ...
                                    <div className="flex items-end">
                                        <label className="flex items-center gap-2 text-sm cursor-pointer">
                                            <input type="checkbox" checked={form.billable} onChange={e => setForm({ ...form, billable: e.target.checked })} className="rounded dark:bg-zinc-950 dark:border-zinc-800" />
                                            <span className="font-medium text-slate-700 dark:text-zinc-300">Facturable</span>
                                        </label>
                                    </div>
                                </div>
                                <div className="flex justify-end gap-2 pt-2">
                                    <Button variant="outline" className="dark:border-zinc-800 dark:text-zinc-400" onClick={() => setShowForm(false)}>Cancelar</Button>
                                    <Button onClick={handleLog} className="bg-teal-600 hover:bg-teal-700 text-white border-0">Registrar</Button>
                                </div>
                            </div>
                        </div>
                    )}
