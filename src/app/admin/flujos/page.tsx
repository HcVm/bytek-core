    ...
    return (
        <div className="p-8 max-w-6xl mx-auto space-y-6">
            <div className="flex justify-between items-end border-b border-zinc-200 pb-4">
                <div>
                    <h1 className="text-3xl font-extrabold tracking-tight text-zinc-900">Flujos y Requerimientos</h1>
                    <p className="text-zinc-500 mt-1">Mesa de ayuda y coordinación interdepartamental.</p>
                </div>
                <RequestFormDialog />
            </div>
            ...
            <div className="pt-4">
                <h2 className="text-lg font-bold text-zinc-800 mb-4">
                    Bandeja Recibida {!primaryDeptId && <span className="text-sm font-normal text-zinc-500">(Esperando un área...)</span>}
                </h2>
                ...
                <div className="space-y-4">
                    {receivedRequests?.map(req => {
                        ...
                        return (
                            <div key={req._id} className="bg-white border rounded-xl p-5 shadow-sm hover:shadow-md transition-all">
                                <div className="flex justify-between items-start">
                                    <div className="flex gap-4">
                                        <div className={`w-12 h-12 rounded-lg flex items-center justify-center shrink-0 border ${req.status === 'resolved' || req.status === 'closed' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-zinc-50 text-zinc-600 border-zinc-200'}`}>
                                            {req.status === 'resolved' || req.status === 'closed' ? <CheckCircle2 className="w-6 h-6" /> : <Clock className="w-6 h-6" />}
                                        </div>
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                <Badge variant="outline" className={`text-[10px] uppercase ${priorityColors[req.priority]}`}>
                                                    Prioridad {req.priority}
                                                </Badge>
                                                <span className="text-xs font-semibold text-zinc-500 px-2 py-0.5 rounded-full bg-zinc-100">
                                            ...
                                            <h3 className="font-bold text-zinc-900 text-lg">{req.title}</h3>
                                            <p className="text-sm text-zinc-600 mt-1 max-w-2xl">{req.description}</p>
                                        </div>
                                    </div>

                                    <div className="flex flex-col items-end gap-2 text-xs">
                                        <div className="flex items-center gap-1.5 text-zinc-600 bg-zinc-50 px-3 py-1.5 rounded-md border border-zinc-100">
                                            <User className="w-4 h-4 text-zinc-400" />
                                            <span>De: <strong>{sender?.name || 'Desconocido'}</strong></span>
                                        </div>
                                        {req.status === 'pending_approval' && (
                                            <span className="flex items-center gap-1 text-amber-600 font-semibold bg-amber-50 px-2 py-1 rounded border border-amber-200">
                                        ...
                                        {assigned ? (
                                            <>
                                                <div className="w-6 h-6 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700">
                                                    <span className="text-[10px] font-bold">{assigned.name?.charAt(0)}</span>
                                                </div>
                                                Atendido por: {assigned.name}
                                            </>
                                        ) : (
                                            <span className="text-amber-600 flex items-center gap-1 bg-amber-50 px-2 py-1 rounded border border-amber-100 text-xs shadow-sm">
                    ...
                    {(!receivedRequests || receivedRequests.length === 0) && (
                        <div className="p-12 text-center border-2 border-dashed border-zinc-200 rounded-2xl text-zinc-500 w-full">
                            <LayoutList className="w-8 h-8 text-zinc-300 mx-auto mb-2" />
