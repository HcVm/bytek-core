                ...
                <div className="flex justify-between items-start max-w-7xl mx-auto w-full">
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                            ...
                            <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">Tesorería y Bancos</h1>
                        </div>
                        <p className="text-slate-500 font-medium pl-14">Gestión de cuentas bancarias, caja chica y flujo de efectivo.</p>
                    </div>
                </div>
            </div>
                ...
                <div className="max-w-7xl mx-auto p-8 space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                            <p className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-1">Posición Total USD</p>
                            <h3 className="text-2xl font-black text-slate-900">$ {totalUSD.toFixed(2)}</h3>
                        </div>
                        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                            <p className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-1">Cuentas Activas</p>
                            <h3 className="text-2xl font-black text-slate-900">{bankAccounts.filter(a => a.isActive).length}</h3>
                        </div>
                    </div>
                        ...
                        {bankAccounts.length === 0 ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {bankAccounts.map((account: any) => (
                                    <div key={account._id} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 hover:shadow-md transition-shadow">
                                        <div className="flex items-center justify-between mb-3">
                                            <div className="flex items-center gap-2">
                                                <CreditCard className="w-5 h-5 text-amber-600" />
                                                <span className="font-bold text-slate-900">{account.bankName}</span>
                                            </div>
                                            <Badge variant="outline" className="text-[10px]">{account.accountType}</Badge>
                                        </div>
                                        <p className="text-xs text-slate-400 font-mono mb-3">{account.accountNumber}</p>
                                        ...
                                        <p className="text-[10px] text-slate-400 mt-2">Cuenta contable: {account.accountingAccountCode}</p>
                                    </div>
                                ))}
                            </div>
                        )}
                    ...
                    {/* Recent transactions */}
                    <div>
                        <h2 className="text-lg font-bold text-slate-900 mb-4">Últimos Movimientos</h2>
                            ...
                            {transactions.length === 0 ? (
                                ...
                                <div className="divide-y divide-slate-50">
                                    {transactions.slice(0, 20).map((tx: any) => (
                                        <div key={tx._id} className="flex items-center gap-4 px-5 py-3 hover:bg-slate-50/80 transition-colors">
                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${tx.type === 'ingreso' ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'}`}>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-[10px] text-slate-400">{tx.date} · {tx.bankName} · {tx.reference || "Sin ref."}</p>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                {tx.reconciled && <Badge className="bg-emerald-100 text-emerald-700 text-[10px]">Conciliado</Badge>}
                                                <span className={`text-sm font-black ${tx.type === 'ingreso' ? 'text-emerald-700' : 'text-red-700'}`}>
